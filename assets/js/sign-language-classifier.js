...
(function (global) {
  ...
  const PARAMS = {
    // ── Smoother ──────────────────────────────────────────────────
    BUFFER_FRAMES:   30,   // Number of frames in the rolling window (~1 s at 30 fps)
    EXP_DECAY:       0.88, // Weight of frame age: recent frames count more (0-1)
    MIN_BUFFER_FILL: 0.55, // Fraction of buffer needed before we can confirm (0-1)
    MIN_CONFIRM:     0.52, // Weighted-vote ratio required to confirm a gesture (0-1)
    COOLDOWN_MS:     2000, // Minimum ms between two fires of the SAME gesture

    // ── Classifier ────────────────────────────────────────────────
    MIN_CONFIDENCE:  0.50, // Raw classify() score below this → gesture:null (Unknown)
    MIN_SEPARATION:  0.10, // Top-vs-second score gap required; below → penalise confidence
  ...
  function extractFeatures(lm) {
    ...
    return { scale, pc, bends, thumb, palmNormal: pn };
  }
  ...
  function scoreHospital(f) {
    ...
    const iExt   = fallsBelow(b.i, ET, SS);
    const mExt   = fallsBelow(b.m, ET, SS);
    const rCurl  = risesAbove(b.r, CT, SS);      // ← ring curled (diff from Water)
    const pCurl  = risesAbove(b.p, CT, SS);
    // Thumb must NOT be spread out (diff from Thank you)
    const tNotOut = fallsBelow(t.toIdx, PARAMS.THUMB_OUT_MIN, 0.09);
    return geo(iExt, mExt, rCurl, pCurl, tNotOut);
  }
