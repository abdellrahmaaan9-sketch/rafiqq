...
function languageIcon(string $language): string {
    ...
    if (str_contains($l, 'arabic')) return '🇸🇦';
    if (str_contains($l, 'english')) return '🇬🇧';
    if (str_contains($l, 'french')) return '🇫🇷';
    if (str_contains($l, 'spanish')) return '🇪🇸';
    if (str_contains($l, 'german')) return '🇩🇪';
    if (str_contains($l, 'italian')) return '🇮🇹';
    if (str_contains($l, 'turkish')) return '🇹🇷';
    return '🌐';
}
...
$languages = [];
try {
    $stmtLang = $pdo->query('SELECT languages FROM interpreter WHERE languages IS NOT NULL AND TRIM(languages) <> \'\'');
...
if ($selected_language !== "") {
    try {
        ...
        foreach ($rows as $row) {
            ...
            $img = !empty($row['photo']) ? $row['photo'] : interpreterIllustration($gender, $selected_language);
...
if (isset($_GET["action"]) && $_GET["action"] === "interpreter_day_slots") {
    ...
    try {
        ...
        foreach ($generated as $slot) {
            ...
            if ($isBooked) $booked[] = $cleanSlot;
            else $available[] = $cleanSlot;
        }
        ...
        echo json_encode([
            ...
            "provider_icon" => $interpreterIconById[$provider_id] ?? "🌐",
    ...
    if ($submit_error === "") {
        ...
        try {
            $sql = "
                INSERT INTO booking
                    (\"date\", booking_time, service_time, start_at, end_at,
                     address, payment_total, payment_method, payment_status, payment_state,
                     patient_id, provider_id, status, service_type, fullname, phone, email)
                VALUES
                    (:date, :booking_time, :service_time, :start_at, :end_at,
                     :address, :payment_total, :payment_method, :payment_status, :payment_state,
                     :patient_id, :provider_id, :status, :service_type, :fullname, :phone, :email)
            ...
            $stmt->execute([
                ...
                ":provider_id" => $provider_id,
                ":status" => "pending",
                ":service_type" => "Interpreter - " . $language,
                ":fullname" => $fullname,
                ":phone" => $phone,
                ":email" => $email,
            ]);
...
<div class="page-wrap">
    <section class="top-box">
        <div class="top-content">
            ...
            <p>Choose the language first, then select your interpreter and pick one available appointment slot. Interpreter pricing is 150 EGP per 30 minutes.</p>
                ...
                <?php foreach($languages as $lang): ?>
                    <label class="type-card <?= $selected_language === $lang ? 'active' : '' ?>">
                        ...
                        <span class="specialty-label">
                            <span class="specialty-icon"><?= h(languageIcon($lang)) ?></span>
                            <span><?= h($lang) ?></span>
                        </span>
                    </label>
                <?php endforeach; ?>
        ...
        <section id="step3Section" class="simple-card">
            ...
            <div class="duration-box">
                ...
                <select id="durationSelect">
                    <option value="30">30 minutes — 150 EGP</option>
                    <option value="60">1 hour — 300 EGP</option>
                    <option value="90">1 hour 30 minutes — 450 EGP</option>
                    <option value="120">2 hours — 600 EGP</option>
                    <option value="150">2 hours 30 minutes — 750 EGP</option>
                    <option value="180">3 hours — 900 EGP</option>
                    <option value="210">3 hours 30 minutes — 1050 EGP</option>
                    <option value="240">4 hours — 1200 EGP</option>
                    <option value="270">4 hours 30 minutes — 1350 EGP</option>
                    <option value="300">5 hours — 1500 EGP</option>
                    <option value="330">5 hours 30 minutes — 1650 EGP</option>
                    <option value="360">6 hours — 1800 EGP</option>
                    <option value="390">6 hours 30 minutes — 1950 EGP</option>
                    <option value="420">7 hours — 2100 EGP</option>
                    <option value="450">7 hours 30 minutes — 2250 EGP</option>
                    <option value="480">8 hours — 2400 EGP</option>
                </select>

                <div id="pricePreview" class="price-preview">Total: 150 EGP</div>
            </div>
            ...
            <div class="booking-layout">
                ...
                <div class="side-stack">
                    <div class="info-card">
                        <div class="info-title">Selected Start</div>
                        <div class="info-value" id="selectedTimeText">--:--</div>
                        <div class="small-note">Your chosen start time appears here.</div>
                    </div>

                    <div class="info-card">
                        <div class="info-title">Session Ends</div>
                        <div class="info-value" id="selectedEndText">--:--</div>
                        <div class="small-note">Price is 150 EGP per 30 minutes.</div>
                    </div>

                    <div class="selected-doctor-box">
                        <div class="selected-doctor-title">Selected Interpreter</div>
...
function updatePricePreview(){
    ...
    pricePreview.innerText = `Total: ${total} EGP`;
            ...
            <label class="doctor-card ${selectedClass}">
                ...
                <div class="doctor-top">
                    ...
                    <div>
                        ...
                        <div class="doctor-sub"><span>${escapeHtml(item.icon || '🌐')}</span><span>${escapeHtml(item.languages)}</span></div>
                    </div>
                </div>

                <div class="doctor-stats">
                    <span class="stat-pill">⭐ ${escapeHtml(item.rating || 'New')}</span>
...
function updateSelectionSummary(){
    ...
    selectionSummary.innerHTML = `
        ...
        Total: <strong>${escapeHtml(getSelectedTotal())} EGP</strong><br>
        ...
        <div class="selected-doctor-content">
            ...
            <div>
                ...
                <div class="selected-doctor-meta">
                    <div>${escapeHtml(selectedProviderIcon)} ${escapeHtml(selectedProviderLanguages)}</div>
                    <div>⭐ ${escapeHtml(selectedProviderRating || 'New')}</div>
                    <div>${selectedProviderGender === 'female' ? 'Female interpreter' : 'Male interpreter'}</div>
                    <div>${escapeHtml(selectedProviderEmail || '')}</div>
                </div>
                <div class="selected-price">${escapeHtml(selectedLanguage)} · ${getSelectedTotal()} EGP</div>
            </div>
        </div>
    `;
    ...
    for(let i=1; i<=daysInMonth; i++){
        ...
        if(dateStr < TODAY){
            ...
            d.onclick = async function(){
                if(!selectedProviderId){
                    alert("Please choose an interpreter first.");
...
function renderSlotsFromLastData(){
    ...
    const filteredAvailable = available.filter(slot => canUseSlotForDuration(slot, booked));
    ...
    if(!booked.length){
        bookedSlotsEl.innerHTML = `<div class="no-slots">No booked appointments for this date.</div>`;
    } else {
        bookedSlotsEl.innerHTML = booked.map(slot => `
            <button type="button" class="slot-chip booked" disabled>
                ${slot.from} → ${slot.to}
            </button>
        `).join("");
    }
...
async function fetchDaySlots(){
    ...
    try{
        ...
        slotsTitle.innerText = `${data.provider_icon || '🌐'} ${data.provider_name || 'Interpreter'} Schedule`;
        slotsSubtitle.innerText = `Choose one available time below. Duration changes the end time and price automatically.`;
...
document.getElementById("bookingForm").addEventListener("submit", function(e){
    ...
    if(!selectedDateInput.value || !bookingTimeHidden.value || !serviceTimeHidden.value){
        e.preventDefault();
        alert("Please choose an available start time first.");
