...
if ($patient_id <= 0) {
  $error = "This logged-in user is not registered in the patient table.";
}
...
if (isset($_POST['submit_request'])) {
  if ($patient_id <= 0) {
    $error = "Patient record was not found for the logged-in user.";
  } elseif (!$driverCol) {
    ...
    <div class="search-hero">
      ...
      <div class="search-bar-wrap">
        ...
        <input
          ...
          class="search-input"
          placeholder="Search destination (e.g. Mall of Arabia, Cairo…)"
          autocomplete="off"
        >
...
<script>
  ...
  async function setDestinationPoint(lat, lng, label = 'Destination', explicitAddress = '') {
    const fixedLat = +Number(lat).toFixed(7);
    const fixedLng = +Number(lng).toFixed(7);
  ...
  async function fetchNominatim(query) {
    try {
      const url = `https://nominatim.openstreetmap.org/search?format=jsonv2&limit=5&addressdetails=1&q=${encodeURIComponent(query.trim())}`;
      const res = await fetch(url, {
  ...
  function renderSuggestions(results) {
    ...
    acDropdown.innerHTML = results.map((r, i) => {
      const icon = r.source === 'db' ? '🏢' : '📍';
      return `<div class="ac-item" data-idx="${i}" data-lat="${r.lat}" data-lng="${r.lng}" data-name="${encodeURIComponent(r.name)}">
    ...
    acDropdown.querySelectorAll('.ac-item').forEach(item => {
      item.addEventListener('click', async () => {
        ...
        acDropdown.classList.remove('open');
        await setDestinationPoint(lat, lng, 'Destination', name);
  ...
  async function updatePrice(force = false){
    ...
    try{
      ...
      const total = Math.round((km * 8) * 100) / 100;
