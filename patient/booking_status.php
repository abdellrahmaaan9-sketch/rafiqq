...
$serviceIcon = $serviceIcons[strtolower($serviceType)] ?? '📋';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
:root{
    ...
    --amber:#d97706;
    --amber-bg:#fffbeb;
    --shadow:0 28px 72px rgba(41,43,74,.14);
    --shadow-soft:0 14px 36px rgba(41,43,74,.08);
}
*{ box-sizing:border-box; margin:0; padding:0; }
body{
    font-family:"Nunito",system-ui,sans-serif;
    background:
        radial-gradient(circle at 15% 0%, rgba(100,112,210,.18), transparent 30%),
        radial-gradient(circle at 88% 6%, rgba(53,59,105,.12), transparent 26%),
        linear-gradient(180deg,#f5f6ff 0%,#f2f4fa 100%);
    min-height:100vh;
    color:#23243a;
}
.container{
...
.hero{
    ...
    position:relative;
    overflow:hidden;
    box-shadow:0 24px 60px rgba(53,59,105,.32);
}
.hero:after{
    content:"";
    position:absolute;
...
.hero-label{ font-size:13px; font-weight:800; opacity:.78; letter-spacing:.6px; text-transform:uppercase; }
.hero-title{ font-size:30px; font-weight:900; line-height:1.1; margin-top:4px; }
.hero-sub{ margin-top:6px; font-size:15px; font-weight:700; opacity:.82; }
...
.status-banner.pending{  background:rgba(255,255,255,.12); border:1px solid rgba(255,255,255,.22); color:#fff; }
.status-banner.accepted{ background:rgba(22,134,83,.22);  border:1px solid rgba(22,134,83,.35);  color:#c8fce4; }
.status-banner.completed{ background:rgba(22,134,83,.22); border:1px solid rgba(22,134,83,.35);  color:#c8fce4; }
.status-banner.declined{ background:rgba(220,38,38,.22);  border:1px solid rgba(220,38,38,.35);  color:#fecaca; }
...
.card-title{
    font-size:18px; font-weight:900; color:#20213b; margin-bottom:16px;
...
.avatar{
    ...
    border-radius:18px;
    background:linear-gradient(135deg,#292b4a,#353b69);
    color:#fff;
    display:grid; place-items:center;
...
.detail-cell{
    background:#f8f8ff;
    border:1px solid rgba(100,112,210,.1);
    border-radius:16px;
    padding:13px 14px;
}
.detail-label{ font-size:11px; font-weight:900; color:#8b91a6; text-transform:uppercase; letter-spacing:.7px; margin-bottom:5px; }
.detail-value{ font-size:15px; font-weight:900; color:#30324c; }
...
.amount-big{
    font-size:36px; font-weight:900; color:#20213b; letter-spacing:-.6px;
}
.method-pill{
...
@keyframes pulse-dot{
    0%,100%{ box-shadow:0 0 0 0 rgba(100,112,210,.4); }
    50%    { box-shadow:0 0 0 8px rgba(100,112,210,0); }
}
...
.tl-dot.done .tl-check{ display:block; }
...
.btn-primary{
    ...
    transition:.2s ease; flex:1; min-width:140px;
}
.btn-primary:hover{ opacity:.9; transform:translateY(-2px); }
.btn-secondary{
    display:inline-flex; align-items:center; justify-content:center; gap:8px;
    padding:14px 22px; border-radius:18px;
    background:#fff; color:#353b69;
...
.btn-secondary:hover{ background:#f8f8ff; }
...
/* ── confetti card ── */
.celebrate{
    background:linear-gradient(135deg,#eefbf4,#f0fbf5);
    border:1px solid rgba(22,134,83,.2);
    border-radius:28px;
    padding:24px;
    text-align:center;
    margin-bottom:18px;
...
.celebrate-icon{ font-size:52px; margin-bottom:10px; }
...
.star-rating label:hover ~ label{ color:#f4b400; transform:translateY(-2px) scale(1.06); }
...
.rate-btn{
    ...
    font-size:14px;
    font-family:inherit;
    box-shadow:0 10px 24px rgba(53,59,105,.22);
    transition:.2s ease;
}
.rate-btn:hover{ transform:translateY(-1px); box-shadow:0 14px 28px rgba(53,59,105,.28); }
...
<main class="container">
    ...
    <div class="hero">
        <div class="hero-top">
            <div class="service-badge"><?= $serviceIcon ?></div>
            <div>
                <div class="hero-label">Booking #<?= (int)$booking_id ?></div>
                <div class="hero-title"><?= h($serviceType) ?> Request Submitted</div>
                <div class="hero-sub">We're finding you the best provider — hang tight!</div>
            </div>
        </div>
        ...
        <div class="card-inner">
            ...
            <div class="details-grid">
                <div class="detail-cell">
                    ...
                    <div class="detail-value">#<?= (int)$booking_id ?></div>
                </div>
                <div class="detail-cell">
    ...
    <div class="card">
        <div class="card-inner">
            ...
            <div class="timeline" id="timeline">
                <?php
                $steps = [
                    ['icon' => '✅', 'title' => 'Booking confirmed',    'sub' => 'Your request has been sent',              'key' => 'confirmed'],
                    ['icon' => '🔍', 'title' => 'Provider reviewing',   'sub' => 'A provider is looking at your request',   'key' => 'reviewing'],
                    ['icon' => '🤝', 'title' => 'Provider accepted',    'sub' => 'Your provider is on the way',             'key' => 'accepted'],
                    ['icon' => '⭐', 'title' => 'Service completed',    'sub' => 'Enjoy rating your experience',            'key' => 'completed'],
                ];
                $activeKey = 'confirmed';
...
(function(){
    ...
    const bannerConfig = {
        pending   : { cls:'pending',   icon:'⏳', title:'Waiting for a provider to accept',      sub:'Your request is live and visible to providers' },
        accepted  : { cls:'accepted',  icon:'✅', title:'Provider accepted your request!',        sub:'They are preparing to come to you' },
        arrived   : { cls:'accepted',  icon:'🚗', title:'Your provider has arrived!',             sub:'They are waiting for you' },
        in_trip   : { cls:'accepted',  icon:'🟢', title:'You are on your way!',                  sub:'Enjoy your trip' },
        in_session: { cls:'accepted',  icon:'🟢', title:'Session in progress',                   sub:'Your provider is with you' },
        completed : { cls:'completed', icon:'🎉', title:'Service completed!',                    sub:'We hope everything went smoothly' },
        declined  : { cls:'declined',  icon:'❌', title:'Request declined',                      sub:'The provider was unable to accept. Please try again.' },
        cancelled : { cls:'declined',  icon:'❌', title:'Booking cancelled',                     sub:'This booking has been cancelled' },
    };
