...
html, body { height: 100%; font-family: "Nunito", system-ui, sans-serif; background: var(--bg); color: var(--text); }
...
.topbar {
    ...
    box-shadow: 0 2px 10px rgba(30,32,64,.06);
}
.back-btn {
...
.back-btn:hover  { background: var(--light); border-color: var(--accent); transform: translateX(-2px); }
...
.intro-hero {
    ...
    background: linear-gradient(135deg, var(--navy) 0%, var(--purple) 55%, var(--accent) 100%);
...
.msg-row.bot  .msg-av { background: linear-gradient(135deg, var(--purple), var(--accent)); }
.msg-row.user .msg-av { background: linear-gradient(135deg, #2f3060, #4a3fa0); }
...
/* ── Bottom panel ────────────────────────────────────────── */
.bottom { flex-shrink: 0; background: var(--card); border-top: 1px solid var(--border); box-shadow: 0 -4px 22px rgba(30,32,64,.05); }
...
.chip {
    ...
    border: 1.5px solid rgba(100,112,210,.2); background: var(--light);
...
.chip:hover  { background: #d8dcff; border-color: var(--accent); transform: translateY(-1px); }
...
<div class="page">
    ...
    <header class="topbar" role="banner">
        <a href="<?= htmlspecialchars($back_link) ?>" class="back-btn" aria-label="Return to homepage">
    ...
    <div class="bottom">
        <div class="chips-wrap" aria-label="Quick question suggestions">
            ...
            <div class="chips-scroll">
                <button class="chip" data-q="How do I find accessible places near me?">🗺️ Find accessible places</button>
                <button class="chip" data-q="How do I book a driver?">🚗 Book a driver</button>
                <button class="chip" data-q="How do I book a doctor for a home visit?">👨‍⚕️ Book a doctor</button>
                ...
                <button class="chip" data-q="I am visually impaired. Is Rafiq accessible?">👁️ Visual impairment</button>
                ...
                <button class="chip" data-q="How do I report incorrect information on the map?">⚠️ Report wrong info</button>
                <button class="chip" data-q="How can I join Rafiq as a service provider?">💼 Join as provider</button>
                <button class="chip" data-q="Is my personal data safe on Rafiq?">🔒 Privacy & safety</button>
...
function addUserBubble(text) {
    ...
    row.innerHTML = `
        <div class="msg-av" aria-hidden="true">🙋</div>
...
function showTyping() {
    ...
    row.innerHTML = `
        <div class="msg-av" style="background:linear-gradient(135deg,#353b69,#6470d2);width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0;border:2px solid #fff;box-shadow:0 2px 8px rgba(30,32,64,.11);" aria-hidden="true">🤖</div>
