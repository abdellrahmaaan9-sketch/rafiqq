...
.pnav-map-pill{
    ...
    color:#4b4f83;
    background:#eef2ff;
    border:1.5px solid rgba(100,112,210,.20);
    padding:8px 16px;
    border-radius:12px;
...
.pnav-map-pill:hover{
    background:#e4e9ff;
    border-color:rgba(100,112,210,.38);
    color:#2B2C41;
}

/* Sign Language pill button */
.pnav-sl-pill{
    display:flex;
    align-items:center;
    gap:7px;
...
<nav class="pnav" id="pnav">
    ...
    <div class="pnav-right">
        ...
        <a class="pnav-vc-pill <?= $currentPage === 'voice_companion.php' ? 'active' : '' ?>"
           href="../general/voice_companion.php">
...
(function(){
    ...
    const serviceIcons = { doctor:'🏥', caregiver:'🤲', interpreter:'🗣️', driver:'🚗' };
    ...
    function renderList(){
        ...
        list.innerHTML = notes.slice(0,15).map(n => `
            <a class="pnav-notif-item" href="../patient/booking_status.php?booking_id=${n.booking_id}">
                ...
                <div>
                    <div class="pnav-notif-text">${n.icon} ${n.service} — ${n.label}</div>
                    <div class="pnav-notif-sub">Booking #${n.booking_id}</div>
                </div>
            </a>`).join('');
    }
                ...
                (data.bookings || []).forEach(b => {
                    ...
                    if(os !== ns){
                        ...
                        notes.unshift({
                            ...
                            icon: serviceIcons[(b.service_type||'').toLowerCase()] || '📋',
